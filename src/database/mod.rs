...
impl Database {
    ...
    pub fn index_database(&mut self) {
        ...
        file.seek(SeekFrom::Start(self.first_record_offset as u64))
        ...
        for i in 0..self.record_count {
                ...
                .1
            {
                Value::Character(string) => {
                    Some(string.parse().expect("couldn't parse string to u32"))
                }
    ...
    pub fn get_by_id(&self, id: u32) -> Option<Vec<(String, Value)>> {
        let row_number = match self.index.get(id) {
            0 => return None,
            val => val - 1,
        };
        let mut buffer = Vec::with_capacity(self.record_length);
        ...
        file.take(self.record_length as u64)
            .read_to_end(&mut buffer)
            .expect("failed to read record data");
